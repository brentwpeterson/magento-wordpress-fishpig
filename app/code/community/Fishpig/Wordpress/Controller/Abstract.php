...
#			echo htmlentities(implode("\n", $this->_beforeBodyEndContent));exit;
			if ($beforeBodyEnd = $this->getLayout()->getBlock('before_body_end')) {
				$helper = Mage::helper('wordpress');

				$jQuery = $this->_includejQuery
					&& ($headBlock = $this->getLayout()->getBlock('head'))
					&& strpos(implode(',', array_keys($headBlock->getItems())), 'jquery') === false
					? sprintf('<script type="text/javascript" src="%s"></script>', $helper->getBaseUrl('wp-includes/js/jquery/jquery.js?ver=1.11.3')). sprintf('<script type="text/javascript" src="%s"></script>', $helper->getBaseUrl('wp-includes/js/jquery/jquery-migrate.min.js?ver=1.2.1'))
					: '';

				$beforeBodyEnd->append(
					$this->getLayout()->createBlock('core/text')->setText($jQuery . implode("\n", $this->_beforeBodyEndContent))
				);
			}
		}
